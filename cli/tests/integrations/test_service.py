...
from .common import (assert_command, assert_lines, delete_zk_nodes,
                     ...
                     package_uninstall, service_shutdown, ssh_agent_output,
...
#     assert_lines(['dcos', 'service', 'log', 'marathon'], 10)
...
def test_log_marathon_config():
    returncode, stdout, stderr = exec_command(
        ['dcos', 'service', 'log', 'marathon',
         '--config-file=tests/data/node/ssh_config'])
...
def test_log_marathon():
    stdout, stderr = ssh_agent_output(
        'dcos service log marathon ' +
        '--config-file=tests/data/service/ssh_config')
...
def test_log_config():
    assert_command(
        ['dcos', 'service', 'log', 'chronos', '--config-file=/path'],
        stderr=(b'The `--config-file` argument is invalid for '
