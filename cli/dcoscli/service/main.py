...
Usage:
    ...
    dcos service log [--follow --lines=N --config-file=<path>]
                     <service> [<file>]
...
Options:
    ...
    --config-file=<path>    Path to SSH config file.  Used to access marathon
                            logs.

    --follow                Output data as the file grows

    --inactive              Show inactive services in addition to active ones.
                            Inactive services are those that have been
                            disconnected from master, but haven't yet reached
                            their failover timeout.
...
def _cmds():
    ...
    return [
        ...
        cmds.Command(
            ...
            arg_keys=['--follow', '--lines', '--config-file', '<service>',
                      '<file>'],
            function=_log),
...
def _log(follow, lines, config_file, service, file_):
    ...
    :type lines: int
    :param config_file: SSH config file.  Used for marathon.
    ...
    if service == 'marathon':
        ...
        return _log_marathon(follow, lines, config_file)
    else:
        if config_file:
            raise DCOSException(
                'The `--config-file` argument is invalid for non-marathon '
...
def _log_marathon(follow, lines, config_file):
    ...
    error = ('`dcos service log marathon` runs ssh to '
             'access the systemd journal on the leading marathon.  To enable '
             'this, you must run `ssh-agent` so your private key can be '
             'used safely to hop between nodes in your cluster.')
    ssh_options = util.get_ssh_options(config_file, [], error)
    ...
    cmd = ("ssh -A -t {3} {0}@{1} " +
           "ssh -A -t {0}@{2} " +
           "journalctl {4} -u marathon").format(
               user,
               master_public_ip,
               leader_ip,
               ssh_options,
