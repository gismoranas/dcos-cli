...
def _ssh(master, slave, option, config_file, user):
    ...
    error = ('`dcos node ssh` depends on `ssh-agent` so your private key '
             'can be used safely to hop between nodes in your cluster.')
    ssh_options = util.get_ssh_options(config_file, option, error)
